...
export async function handleTabsRequest(
  service: AutoBrowserService,
  request: IncomingMessage,
  response: ServerResponse
) {
  const body = await readJsonBody<CommandPayloadMap["tabs"]>(request);
  const result = await service.dispatchCommand("tabs", body);
