...
export async function handleMouseTrajectoryDeleteRequest(
  service: AutoBrowserService,
  request: IncomingMessage,
  response: ServerResponse
) {
  const body = await readJsonBody<CommandPayloadMap["mouseTrajectoryDelete"]>(request);
  const result = await service.dispatchCommand("mouseTrajectoryDelete", body);
