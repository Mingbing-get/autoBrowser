...
export async function handleMouseTrajectoryCreateRequest(
  service: AutoBrowserService,
  request: IncomingMessage,
  response: ServerResponse
) {
  const body = await readJsonBody<CommandPayloadMap["mouseTrajectoryCreate"]>(request);
  const result = await service.dispatchCommand("mouseTrajectoryCreate", body);
