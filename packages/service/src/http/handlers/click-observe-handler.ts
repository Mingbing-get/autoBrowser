...
export async function handleClickObserveRequest(
  service: AutoBrowserService,
  request: IncomingMessage,
  response: ServerResponse
) {
  const body = await readJsonBody<CommandPayloadMap["clickObserve"]>(request);
  const result = await service.dispatchCommand("clickObserve", body);
