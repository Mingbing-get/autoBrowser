...
export async function handleSearchFromPointRequest(
  service: AutoBrowserService,
  request: IncomingMessage,
  response: ServerResponse
) {
  const body = await readJsonBody<CommandPayloadMap["searchFromPoint"]>(request);
  const result = await service.dispatchCommand("searchFromPoint", body);
